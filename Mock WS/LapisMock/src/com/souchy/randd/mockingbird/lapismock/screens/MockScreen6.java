...
public class MockScreen6 extends SapphireSetupScreen {
	...
	public MockScreen6() {
		...
		//shadersSsao = new SSAOShaderProvider(LapisShader.getVertexShader("base"), LapisShader.getFragmentShader("base"));
		//ssaoFBO = new FrameBuffer(Format.RGBA8888, 1600, 900, false); // Gdx.graphics.getWidth(), Gdx.graphics.getWidth(), false);
		createSSAOFBO(1600, 900);
		ssaoModelBatch = new ModelBatch(shadersSsao);
		Log.info("MockScreen 6 b");

		this.controller.keyCombos.put(Keys.CONTROL_LEFT, Keys.SPACE, () -> {
			shadersSsao.clear();
			shadersAdv.clear();
		});
	}
	
	private void createSSAOFBO(int width, int height) {
