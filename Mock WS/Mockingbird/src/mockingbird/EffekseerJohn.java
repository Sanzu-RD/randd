...
public class EffekseerJohn extends ApplicationAdapter implements InputProcessor {
	SpriteBatch batch;
	Texture img;
	...
	public static void main(String[] arg) {
		...
		new LwjglApplication(new EffekseerJohn(), config);
	}
	...
	public void create() {
		...
		viewport = new FitViewport(1280, 720, perspectiveCamera);
...
//		manager.setViewport(viewport);
		...
		try {
			effekseer.load("data/tu.efk", true);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void loadObjs() {
		...
		effekseer.setLocation(50, 0, 2.7f);
	...
	public boolean keyDown(int keycode) {
		...
		if(keycode == Input.Keys.W) {
			...
			effekseer.setLocation(0f, -0.5f, 3.1832256f);
			
		}
		
		if(keycode == Input.Keys.S) {
			
			effekseer.setLocation(effekseer.getX(), effekseer.getY() - (2 * Gdx.graphics.getDeltaTime()), effekseer.getZ());
		...
		if(keycode == Input.Keys.Z) {
			...
			orthographicCamera.position.set(orthographicCamera.position.x, (orthographicCamera.position.y += (50 * Gdx.graphics.getDeltaTime())), 0);
			
		}
		
		if(keycode == Input.Keys.T) {
			
			perspectiveCamera.position.set(perspectiveCamera.position.x, (perspectiveCamera.position.y += Gdx.graphics.getDeltaTime()),
