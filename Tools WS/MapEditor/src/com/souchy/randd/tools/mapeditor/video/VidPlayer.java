...
import com.badlogic.gdx.video.VideoPlayerMesh;
...
public class VidPlayer implements VideoPlayer  {
    ...
    public VidPlayer(Viewport viewport) {
        this(viewport.getCamera(), new VideoPlayerMesh());

        this.viewport = viewport;
    }

    public VidPlayer(Camera cam, Mesh mesh, int primitiveType) {
        this(cam, VideoPlayerMesh.fromCustomMesh(mesh, primitiveType));
    }

    private VidPlayer(Camera cam, VideoPlayerMesh mesh) {
        this.cam = cam;
        this.mesh = mesh;
    }
