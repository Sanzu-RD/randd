 ...
 * @author mzechner
 * @author Nathan Sweet */
public class SapphireBatch implements Batch {
	...
	/** Constructs a new SapphireBatch with a size of 1000, one buffer, and the default shader.
	 * @see SapphireBatch#SapphireBatch(int, ShaderProgram) */
	public SapphireBatch () {
		this(1000, null);
	}

	/** Constructs a SapphireBatch with one buffer and the default shader.
	 * @see SapphireBatch#SapphireBatch(int, ShaderProgram) */
	public SapphireBatch (int size) {
		this(size, null);
	}

	/** Constructs a new SapphireBatch. Sets the projection matrix to an orthographic projection with y-axis point upwards, x-axis
	 ...
	 * @param defaultShader The default shader to use. This is not owned by the SapphireBatch and must be disposed separately. */
	public SapphireBatch (int size, ShaderProgram defaultShader) {
		...
		short j = 0;
		for (int i = 0; i < len; i += indicesPerQuad, j += verticesPerQuad) {
			indices[i] = j;
	...
	/** Returns a new instance of the default shader used by SapphireBatch for GL2 when no shader is specified. */
	...
	@Override
	public void begin () {
		if (drawing) throw new IllegalStateException("SapphireBatch.end must be called before begin.");
		renderCalls = 0;
	...
	@Override
	public void end () {
		if (!drawing) throw new IllegalStateException("SapphireBatch.begin must be called before end.");
		if (idx > 0) flush();
		lastTexture = null;
		drawing = false;
...
//		return color;
	...
	public void draw (Texture texture, float x, float y, float originX, float originY, float width, float height, float scaleX,
		...
		if (!drawing) throw new IllegalStateException("SapphireBatch.begin must be called before draw.");
...
//			idx += VERTEX_SIZE;
	...
	/**
	 * Draw a fucking circle texture
	 */
	public void draw (RoundTextureRegion region, float x, float y) {
	 ...
	 * Draw a fucking circle texture
	 */
	public void draw (RoundTextureRegion region, float x, float y, float width, float height) {
		draw(region, x, y, width, height, 1.0f);
	}
	...
	private void draw(TextureRegion region, float x, float y, float width, float height, float type) {
		if (!drawing) throw new IllegalStateException("SapphireBatch.begin must be called before draw.");
