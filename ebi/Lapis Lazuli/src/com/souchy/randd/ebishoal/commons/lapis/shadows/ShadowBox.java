package com.souchy.randd.ebishoal.commons.lapis.shadows;
...
public class ShadowBox {
    ...
    protected void update() {
        ...
        toNear.scale(cam.near); //MasterRenderer.NEAR_PLANE);
        var pos = new Vector3f(cam.position.x, cam.position.y, cam.position.z);
    ...
    private Matrix4f calculateCameraRotationMatrix() {
		var dx = cam.direction.x;
		var dy = cam.direction.y;
		var dz = cam.direction.y;
		var a = Math.asin(dx / dz);
		var b = Math.asin(dy / dz);
		
        Matrix4f rotation = new Matrix4f();
        rotation.rotate((float) Math.toRadians(a), new Vector3f(0, 1, 0)); // -cam.getYaw()
        rotation.rotate((float) Math.toRadians(b), new Vector3f(1, 0, 0)); // -cam.getPitch()
    ...
    private void calculateWidthsAndHeights() {
    	float fieldOfView;
    	if(cam instanceof PerspectiveCamera) fieldOfView = ((PerspectiveCamera) cam).fieldOfView;
    	else fieldOfView = 1;
        farWidth = (float) (SHADOW_DISTANCE * Math.tan(Math.toRadians(fieldOfView)));
        nearWidth = (float) (cam.near * Math.tan(Math.toRadians(fieldOfView)));
    ...
    private float getAspectRatio() {
        return Gdx.graphics.getWidth() / Gdx.graphics.getHeight(); //(float) Display.getWidth() / (float) Display.getHeight();
    }
